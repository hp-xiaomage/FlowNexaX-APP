...
// ========== 读取远端Excel（明道云），会阻塞等待返回 ==========
var result4 = readExcel(JSON.stringify({
  remote: true,                    // true表示读取远端
  worksheetId: '你的worksheetId',  // 明道云工作表ID
  appKey: '你的appKey',            // 明道云AppKey
  sign: '你的sign',                // 明道云Sign
  viewId: '你的viewId',           // 视图ID，可不填
  row: 1,                          // 行号从1开始；-1表示读最后一行
  fieldKeys: ['姓名', '手机号', '状态']  // 要读取的字段名数组
}));
...
if (!r2.success) {
  ...
  log('读取行号：' + r2.row);
  log('行ID：' + r2.rowId);          // 该行在明道云的唯一ID，更新时可用于定位
  log('姓名：' + r2.values[0]);
  log('手机号：' + r2.values[1]);
  log('状态：' + r2.values[2]);
}
...
// ========== 写入远端Excel（row=-1 追加新行）==========
var result5 = writeExcel(JSON.stringify({
  remote: true,
  worksheetId: '你的worksheetId',
  appKey: '你的appKey',
  sign: '你的sign',
  row: -1,                          // -1表示追加新行
  fields: [
    {fieldKey: '姓名',   value: '王五'},   // fieldKey字段名需与明道云一致
    {fieldKey: '手机号', value: '13900000000'},
    {fieldKey: '状态',   value: '待处理'}
  ]
}));
...
if (!w3.success) {
  ...
  log('追加成功，行ID：' + w3.rowId);
  log('是否追加新行：' + w3.append);   // true=追加新行，false=更新已有行
}
...
if (!w4.success) {
  ...
  log('更新成功，行号：' + w4.row + '，行ID：' + w4.rowId);
}


// ========== 综合示例：逐行读本地Excel，写入远端明道云 ==========
var rowNum = 2;  // 从第2行开始，第1行通常是表头
...
while (true) {
  ...
  if (!rd.success) {
    log('第' + rowNum + '行读取结束或出错：' + rd.msg);
    break;
  }

  log('读取第' + rowNum + '行：' + rd.values[0] + ' / ' + rd.values[1] + ' / ' + rd.values[2]);
  ...
  var writeResult = writeExcel(JSON.stringify({
    ...
    row: -1,
    fields: [
      {fieldKey: '姓名',   value: rd.values[0]},
      {fieldKey: '手机号', value: rd.values[1]},
      {fieldKey: '备注',   value: rd.values[2]}
    ]
  }));
  ...
  if (!wd.success) {
    log('第' + rowNum + '行上传失败：' + wd.msg);
  } else {
    log('第' + rowNum + '行上传成功，行ID：' + wd.rowId);
  }
